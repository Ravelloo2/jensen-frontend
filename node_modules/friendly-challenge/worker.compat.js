    ...
    return new Promise(function ($return, $error) {
      ...
      try {
        ...
        if (data.type === "solver") {
          ...
          if (data.type === "start") {
            var solve, totalH, starts, solutionBuffer, doneMessage;
            ...
            return Promise.resolve(solver).then(function ($await_13) {
              try {
                ...
                totalH = 0;
                starts = data.puzzleSolverInputs;
                solutionBuffer = new Uint8Array(8 * data.n); // In the case of 4 workers, the first worker will solve puzzle
                // 0, 4, 8, 12 etc

                for (var puzNum = data.startIndex; puzNum < starts.length; puzNum += data.numWorkers) {
                  var solution = void 0; // We loop over a uint32 to find as solution, it is technically possible (but extremely unlikely - only possible with very high difficulty) that
                  // there is no solution, here we loop over one byte further up too in case that happens.

                  for (var b = 0; b < 256; b++) {
                    starts[puzNum][123] = b;

                    var _solve = solve(starts[puzNum], data.threshold),
